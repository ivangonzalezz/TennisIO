...
public var persistentContainer: NSPersistentContainer = {
    let container = NSPersistentContainer(name: "TennisData")
    container.loadPersistentStores(completionHandler: { (storeDescription, error) in
        if let error = error as NSError? {
            fatalError("Unresolved error \(error), \(error.userInfo)")
        }
...
let fetchRequest: NSFetchRequest<TennisDataEntity> = NSFetchRequest(entityName: "TennisDataEntity")
...
func getCount() -> Int {
    ...
    print("The fetch count is \(count)")
    var managedObject = NSManagedObject(entity: persistentContainer.managedObjectModel.entitiesByName["TennisDataEntity"] ?? NSEntityDescription(), insertInto: managedObjectContext)
    managedObject.setValue(1, forKey: "xRotation")
    managedObject.setValue(2, forKey: "yRotation")
    managedObject.setValue(3, forKey: "zRotation")
    print("Object is inserted: \(managedObject.isInserted)")
    do {
